...
//   const [itemFiles, setItemFiles] = useState<ItemFiles>({});
//   const [options, setOptions] = useState<string[]>([]);
//   const [itemResponse, setItemResponse] = useState<ItemInfo | any>();
...
//     // CRITICAL: Check if these are actually File objects before uploading
//     if (objectGLBFile instanceof File) {
//       console.log("Uploading objectGLBFile which IS a File object.");
//       console.log("Internal type property:", objectGLBFile.type); // What does this log?
//       // ... proceed with upload using objectGLBFile
//     } else {
//       console.error("objectGLBFile is NOT a File object!", objectGLBFile);
//       // Handle error - don't attempt upload
//       return { error: "Invalid GLB file data received on server." };
//     }

//     if (objectGLBFile && objectUSDZFile) {
//       const objectFiles: ItemFiles = {
//         objectGLB: objectGLBFile, // Use the file name
//         objectUSDZ: objectUSDZFile, // Use the file name
//       };
//       if (!itemResponse || !itemResponse.data) {
//         toast.error("Primero debes completar los detalles del plato.");
//         return;
//       }

//       try {
//         // setItemFiles(objectFiles);

//         if (objectFiles && itemResponse) {
//           await uploadItemFiles(objectFiles, itemResponse.data[0]);
//         }
//       } catch (error) {
...
//       image_plate: formData.get("image_plate") as File | null, // Placeholder for file handling
//     };
//     try {
...
//         toast.success("Detalles del plato añadidos correctamente.");
//       }
//     } catch (error) {
//       console.log(error);
//       toast.error("Lo siento, hubo un error al añadir el plato");
//     }
//   }
...
//             <TabsTrigger value="files">Archivos 3D</TabsTrigger>
//           </TabsList>
...
//               <button className="cursor-pointer px-4 py-2 text-zinc-100 bg-zinc-700 rounded-2xl transition-all duration-100 ease-in-out active:scale-110">
//                 siguiente
//               </button>
//             </form>
//           </TabsContent>
    ...
    startDetailsTransition(async () => {
      try {
        ...
        if (response?.error || !response?.data?.id) {
          throw new Error(
            response?.error || "No se pudo crear el plato o obtener ID."
          );
        }
